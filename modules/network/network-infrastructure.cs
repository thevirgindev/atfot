...
public class InfrastructureCmd : InteractionModuleBase<SocketInteractionContext>
{
        ...
        [Summary("export", "export format (none, txt, json)")] string export = "none")
    {
        ...
        if (isOnion)
        {
            // Tor required for direct .onion access; we provide Ahmia search.
            ...
            if (_tor != null)
            {
                // Optionally enable Tor for API calls? Not needed for links.
            }
        }

        dto.DeepLinks = links;

        // Attempt Shodan API if key exists
        ...
        {
            dto.ExtractedData["shodan"] = "No API key. Use /admin setkey shodan <key> to enable live data.";
        }
        ...
        [Summary("export", "export format (none, txt, json)")] string export = "none")
    {
        ...
        // Attempt SecurityTrails API if key exists
        ...
        if (!string.IsNullOrEmpty(secTrailsKey))
        {
            try
            {
                ...
                if (response.IsSuccessStatusCode)
                {
                    ...
                    if (subdomains != null)
                    {
                        var subList = string.Join(", ", subdomains.Take(20)); // limit
        ...
        {
            dto.ExtractedData["securitytrails"] = "No API key. Use /admin setkey securitytrails <key> for subdomain enumeration.";
        }
