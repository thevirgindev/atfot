...
public class ContactCmd : InteractionModuleBase<SocketInteractionContext>
{
        ...
        [Summary("export", "export format (none, txt, json)")] string export = "none")
    {
        ...
        if (isEmail)
        {
            ...
            // Try EmailRep API if key present
            ...
            {
                dto.ExtractedData["emailrep"] = "No API key. Use /admin setkey emailrep <key> to enable automated checks.";
            }
        }
        else
        {
            ...
            // Try Truecaller API (requires RapidAPI key? Not free usually)
            ...
            {
                // This would require a specific endpoint; for brevity, we just note.
