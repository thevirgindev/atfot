...
public class IdentityCmd : InteractionModuleBase<SocketInteractionContext>
{
        ...
        [Summary("export", "export format (none, txt, json)")] string export = "none")
    {
        ...
        // Attempt to scrape Namechk results? Namechk blocks automated requests, but we can provide link.
        ...
        var description = $"**Username:** `{username}`\n\n**Investigation Links:**\n" +
                          string.Join("\n", links) +
                          "\n\n*Note: Some tools require manual interaction. Use `/admin setkey` to add API keys for automated results.*";
